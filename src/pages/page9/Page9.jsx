...
import { Electron, ElectronF, ELECTRONIC_URL } from "../../config";
...
export default function Page9() {
  ...
  const searchAd = async (event) => {
    ...
    var { fprice, lprice, condition, type, brand } = document.forms[1];
    ...
    setAds(await fetchSearchAD(ELECTRONIC_URL, request))
  }

  const getImages = async () => {
    setImages(await fetchImage(Electron))
  }

  const getCompany = async () => {
    setCompany(await fetchImage(ElectronF))
  }

  const getAds = async () => {
    setAds(await fetchPageAd(ELECTRONIC_URL))
  }
      ...
      <div className="fill-container d-flex mt-2">
        ...
        <form onSubmit={searchAd} className="search">
          <div className="position-relative btn-icon">
            <span className="mr-1 fs-1">الكترونيات وأجهزة منزلية</span>
