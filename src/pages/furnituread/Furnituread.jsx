...
import { Add, AD_URL, Edit, FURNITURE_URL } from "../../config";
import API from "../../API"
...
export default function Page23() {
  ...
  const handleSubmit = async (event) => {
    ...
    var { ad_name,condition,department,attach,price,description } = document.forms[0];    
    if (!isUserLogin) {
    ...
    request.append('ad_type', "furniture")
    request.append('price', price.value)
    request.append('condition', condition.value)
    request.append('name', department.value)
    request.append('attach', attach.value)
    request.append('description', description.value)
    request.append('ad_image', img)
    const response = await API.postRequest(FURNITURE_URL, Add, request)
              ...
              <form onSubmit={handleSubmit} className="inputs ">
                ...
                <div className="search mt-3 mb-2 reset-inputs">
                  ...
                  <div className="inputs d-grid-2">
                    ...
                    <div className="input position-relative d-flex ">
                      <input type="text" name="attach" placeholder="النوع" />
                      <img src="./assets/imgs/down-arrow.png" alt="icon" />
                    </div>
                    <div className="input position-relative d-flex ">
