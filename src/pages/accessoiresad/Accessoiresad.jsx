...
import { ACCESS_URL, Add, adNavigate } from "../../config";
import API from "../../API"
...
export default function Page23() {
  ...
  const ad_type = "access"

  const [messages, setMessages] = useState('');
  const handleSubmit = async (event) => {
    event.preventDefault();
    var request = new FormData();
    var { ad_name, description, condition, price, type } = document.forms[1];    
    if (!isUserLogin) {
    ...
    const response = await API.postRequest(ACCESS_URL, Add, request)
